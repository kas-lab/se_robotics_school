...
**Checking your work**: To see if you have done everything correctly, run the following command in the docker container:
...
Within Groot, use the 'Load Project/File' button to open the file ``src/rebet_mirte/rebet_school/trees/qrs_sleep``.
...
1. Edit the tree to add some instances of ``NavigateToPose`` (see bottom left of Groot) actions and the decorator nodes in tandem. You can use the waypoints defined for the PDDL exercise as the poses or any other pose you like.
...
We recommend adding a Sleep action node of a few seconds as the first in your tree, to give you time to make sure everything is ready before the tree starts executing. When you are done, save the tree, get your robot ready.
